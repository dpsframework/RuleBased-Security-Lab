...
|  <img src="images/logoPsBoard.gif" height="60px">   | • updateSNORT.service<br> • updateSNORT.DLoader.class<br> • snort2KBASErules()<br> • REresponseKBASE.class  |  **Image 4**. Every 2 days, an IDS-board agent instance downloads the latest rules from the SNORT community from the Internet and converts them to CLIPS / Jess rules through a simple conversion function. When the agent receives a request from the Analyzer-Class agents, it compares the dates with the update date of the SNORT rules. And submit new rules if applicable.    | 
