...
|  <img src="images/logoPsBoard.gif" height="60px">   | • registerFACTS.service<br> • registerINFORM.service<br> • distpatchANALYSIS.service<br> • REresponseFACTS.class<br> • REresponseINFORM.class<br> • REresponseANALYSIS.class<br> • idsDBGridUpdate( )  | **Image 3**. An IDS-board agent instance handles the requests received from Reader-Class agents and from Analyzer-Class agents through three Listen Services. Those services use communication interaction protocol with implemented _Rational-Effect_ (**RE**) JADE behaviours.   | 
|  <img src="images/logoPsBoard.gif" height="60px">   | • updateRULES.service<br> • SnortRulesDownloader.class<br> • snortRulesToKBASE( )<br> • REresponseKBASE.class  |  **Image 4**. Every 2 days, an IDS-board agent instance downloads the latest rules from the SNORT community from the Internet and converts them to CLIPS / Jess rules through a simple conversion function `snortRulesToKBASE( )`. When the agent receives a request from the Analyzer-Class agents, it compares the dates with the update date of the SNORT rules. And submit them new KBase file if applicable.    | 
