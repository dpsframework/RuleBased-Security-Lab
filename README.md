...
|:--------             |:--------------------- |:---------   |
|  <img src="images/idsboard-01.png" width="170px">   | • nidsDB.service<br> • nidsDB.ShowGrid.class<br> • doPopulate()  | **Image 1**. Board-Class agents can display the log of activities associated with the translation of PCAP files, analyzed files records, alerts found and updated Snort rules. All that information is shown in the grid, using first tab: "Ticket's repository table".   | 
|  <img src="images/idsboard-02.png" width="170px">   | • boardDF.service<br> • boardDF.SDregistrator.class<br> • DFAgentDescription()  | **Image 2**. Each IDS-Board-agent instance is registered in Directory Facilitator (DF) of JADE Platform. The Board-Class agents are critical in a possible real-world deployment. For this reason, these agents self-register in the **DF** agent catalogue and can be recovered thanks to the _fault tolerance_ provided by JADE when the DF Service uses persistence [**11**].  | 
|  <img src="images/logoPsBoard.gif" height="60px">   | • listenPCAP.service<br> • listenANLZ.service<br> • REresponsePCAP.class<br> • REresponseANLZ.class<br> • nidsDBupdate()  | **Image 3**. An IDS-board agent instance handles the requests received from Reader-Class agents and from Analyzer-Class agents through two Listen Services. Those services use communication interaction protocol with implemented _Rational-Effect_ JADE behaviours.   | 
|  <img src="images/logoPsBoard.gif" height="60px">   | • updateSNORT.service<br> • updateSNORT.DLoader.class<br> • snortRulesToKBASE()<br> • REresponseKBASE.class  |  **Image 4**. Every 2 days, an IDS-board agent instance downloads the latest rules from the SNORT community from the Internet and converts them to CLIPS / Jess rules through a simple conversion function. When the agent receives a request from the Analyzer-Class agents, it compares the dates with the update date of the SNORT rules. And submit them new rules if applicable.    | 
...
|:--------             |:--------------------- |:---------   |
|  <img src="images/logoPsNode.gif" height="40px">   | • analysis.service<br> • RErequestANLZ.class<br> • REinformANLZ.class<br> • workingMemroy_Loader()  | **Image 5**. Every 15 minutes Analyzer-Class agents request to an IDS-board-agent for new pending analysis files. If agents receive a file as response, the `workingMemroy_Loader()` function load it in their Working-Memory. The REinformANLZ behaviour-class sends results to IDS-board agent when analysis has finished.  |
|  <img src="images/logoPsNode.gif" height="40px">   | • updateKBASE.service<br> • RErequestKBASE.class<br> • knowledgeBase_Loader()  |  **Image 6**. Every 24 hours Analyzer-Class agents request to an IDS-board agent the lastest updated SNORT rules. If agents receive a file as response, the `knowledgeBase_Loader()` function updates their Knowledge-Base and their engines are restarted. |
...
|:--------             |:--------------------- |:---------   |
|  <img src="images/logoPsMonitorAgent.gif" width="40px">   | • analysis.service<br> • RErequestANLZ.class<br> • REinformANLZ.class<br> • workingMemroy_Loader()  | **Image 7**. Every 15 minutes Analyzer-Class agents request to an IDS-board-agent for new pending analysis files. If agents receive a file as response, the `workingMemroy_Loader()` function load it in their Working-Memory. The REinformANLZ behaviour-class sends results to IDS-board agent when analysis has finished.  |
|  <img src="images/logoPsMonitorAgent.gif" width="40px">   | • pcap.service<br> • pcap.Watchdog.class<br> • REinformPCAP.class<br> • watingForPCAP()<br> • transformToFacts()  |  **Image 8**. Every 5 minutes Reader-Class agents are looking for new .PCAP files on `./var/pending/` diectory. They move to their own directories, open it an transform to expert system _Facts_ plain text file wit `transformToFacts()` function. |
