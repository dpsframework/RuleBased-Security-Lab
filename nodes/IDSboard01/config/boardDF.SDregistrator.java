...
public class SDregistrator extends WakerBehaviour {
	...
	public SDregistrator(Agent a, long t) {
		super(a, t);
	...
	public void onWake(){
		...
		mySd  = new ServiceDescription();
 		...
 		mySd.setName(theNode.myNode().nameOfBoardService);  // "ids-board"
 		mySd.setType("web-service");
	...
	public int onEnd() {
		if (exitValue == 0) {
			outLogger("Behaviour has returned [" + exitValue
					+ "] Board has been not registered in DF.Agent");
		} else {
			outLogger("Behaviour has returned [" + exitValue
					+ "] Board has been registered in DF.Agent");
		}
		return exitValue;
	}
